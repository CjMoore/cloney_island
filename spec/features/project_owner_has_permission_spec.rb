...
describe "project owner has permissions" do
  before(:each) do
    @user = create(:user)
    @role = Role.create(name: "project_owner")
    @user.roles << @role
  ...
  it "to visit create comment" do
    ...
    fill_in "comment[content]", with: "I can make a comment"
    click_on("Submit")
    expect(current_path).to eq("/projects/#{project.slug}")
  end
  it "to visit projects/project-name/funds" do
    project = create(:project)
